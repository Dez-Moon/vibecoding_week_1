...
```
User ──1:1── Board ──1:N── Column ──1:N── Card
```
...
### `User`
One row per signed-up user. For the MVP only the hardcoded `user` row exists (created by the seeder if absent). Real auth lands later; this table currently exists so the foreign key from `Board` is honest.
...
- `id` (uuid, pk)
- `board_id` (uuid, fk → `Board.id`, indexed)
...
- `id` (uuid, pk)
- `column_id` (uuid, fk → `Column.id`, indexed)
...
Both `Column.position` and `Card.position` are tight integers renumbered on every write. Concretely:
...
The seed runs once, on app startup, when the `User` table is empty. It inserts the demo `user` row only — no board, no columns, no cards. The user lands on an empty board and adds their own content via the API.
...
| Endpoint | Touches |
| --- | --- |
| `GET /api/board` | `Board`, `Column`, `Card` for the current user's board |
| `PATCH /api/board` | `Column.title` (rename in place) |
| `POST /api/board/cards` | `Card` (insert + renumber target column) |
| `PATCH /api/board/cards/{id}` | `Card` (update title/details) |
| `DELETE /api/board/cards/{id}` | `Card` (delete + renumber source column) |
| `POST /api/board/cards/{id}/move` | `Card` (move + renumber both source and target columns) |
...
- **Multi-board per user.** Drop the `UNIQUE (user_id)` on `Board`; add a `GET /api/boards` endpoint and a board switcher in the UI.
- **Real auth.** Add a `password_hash` column on `User`, swap the signed-cookie MVP for a hashed credential check. The cookie's signed payload already carries the `User.id`, so the rest of the code is unaffected.
- **Card assignees, labels, due dates.** Add as nullable columns on `Card`; no migration cost since the table is small.
